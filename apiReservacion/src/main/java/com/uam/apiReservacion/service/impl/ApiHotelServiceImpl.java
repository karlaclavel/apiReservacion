...
public class ApiHotelServiceImpl implements ApiHotelService {
    ...
    public ResponseEntity<?> buscarHotelesPorPrecio(BigDecimal precioMinimo, BigDecimal precioMaximo) {
    	UriComponentsBuilder builder = UriComponentsBuilder.fromUriString("http://localhost:8081/api/Hotel/precio?precioMinimo={precioMinimo}&precioMaximo={precioMaximo}");
		String apiHotelURL = builder.buildAndExpand(precioMinimo, precioMaximo).toUriString();
    	ResponseEntity<?> response = restTemplate.getForEntity(apiHotelURL, String.class); 
